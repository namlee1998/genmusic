...
class MusicGenerator:
    def __init__(self, base_dir="generated_songs", segment_dir="segments", mixed_dir="mixed_segments"):
        ...
        self.lyric_model = GPT2LMHeadModel.from_pretrained("SpartanCinder/GPT2-finetuned-lyric-generation").to(self.device)
    ...
    def generate_melody(self, prompt, duration=30):
        musicgen = MusicGen.get_pretrained("facebook/musicgen-medium", device=self.device)
        musicgen.set_generation_params(duration=duration, top_k=250, temperature=1.0)
        wav = musicgen.generate([prompt])[0].cpu().numpy()
        if wav.ndim == 2: wav = np.mean(wav, axis=0)
        if np.max(np.abs(wav)) > 1: wav /= np.max(np.abs(wav))
